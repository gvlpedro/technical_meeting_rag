...
flowchart TD
    ...
    subgraph GRAPH["agents/graph.py — Silver LangGraph (make clarify only; no HTTP surface yet)"]
        ...
        N2["generate_questions<br/>LLM call #1"]
        N3["classify_questions<br/>LLM call #2"]
        D1{"pending_questions?"}
        N4["ask_human<br/>interrupt() · no LLM"]
        N5["synthesize_document<br/>LLM call #3 · per source"]
        N6["critic_document<br/>LLM call #4 · per source, always runs"]
    ...
    M2 -. calls generate_questions_for_batch directly .-> N2
    M3 --> N1

    N2 -. prompt .-> P1["prompting/roles/common/clarification_questions.jinja<br/>+ prompting/roles/common/clarification_template.md<br/>(embedded as ARCHITECTURE_CHANGES)<br/>temperature=0, reasoning_effort=none<br/>retries at temp=0.7 on shallow/ungrounded result"]
    N3 -. prompt .-> P2["agents/prompts.py :: build_classification_prompt<br/>(inline system+user strings, no .jinja file)"]
    N5 -. prompt .-> P3["prompting/roles/common/adr_generator.jinja"]
    N6 -. prompt .-> P4["agents/prompts.py :: build_critic_prompt<br/>(inline system+user strings, no .jinja file)"]

    subgraph UNUSED["prompting/roles/common/ — not called by any code path today"]
