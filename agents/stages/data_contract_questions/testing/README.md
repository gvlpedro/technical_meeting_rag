...
(`agents/stages/architecture_questions/testing/`) identifies which components and data contracts exist; this
...
"action"}]`) standing in for what a real `agents.stages.architecture_questions.testing` run would have
...
Same two-tier split as `agents/stages/architecture_questions/testing/`: free, deterministic checks (structural
...
only; see `agents/stages/architecture_questions/testing/README.md` for why a probabilistic judge doesn't gate
this suite either.
