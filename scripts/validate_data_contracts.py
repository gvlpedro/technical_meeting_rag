#!/usr/bin/env python3
"""This script validates the ODCS data contracts generated by prompts/_legacy/data_contracts.md.
...
`--session` is required, unless you pass specific files as arguments. When given, it looks for
every `*.odcs.json` file under `<dir>/<session>/data_contracts/`.
"""
...
def find_contracts(root: str, session: str) -> list[Path]:
    """Only looks under <root>/<session>/data_contracts/. Never looks at other sessions."""
    base = Path(root) / session / "data_contracts"
    return sorted(base.rglob(CONTRACT_GLOB))


def lint_one(path: Path) -> tuple[bool, list[str]]:
    """Runs `datacontract lint` on a single contract file.

    Returns (is_valid, reasons). `reasons` is empty when the file is valid.
    """
    ...
    try:
        ...
        if not is_valid and not reasons:
            # If the JSON report is not available, fall back to whatever the CLI printed.
            reasons = [
                line
