"""Redirects `generate_architecture_questions_for_batch`'s `output/` writes to this directory's own
...
`testing_arch_questions_acb/output/result.json` once the whole run finishes. The full
...
`generate_architecture_questions_for_batch` itself); `result.json` isn't a second copy of that, it's
...
def pytest_terminal_summary(terminalreporter: pytest.TerminalReporter) -> None:
    ...
    if not result_path.exists():
        return
    terminalreporter.section("testing_arch_questions_acb/output/result.json")
