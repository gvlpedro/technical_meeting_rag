...
from agents.schemas import ArchitectureChangeType, ComponentStatus, ContractAction, GoldEntityType
...
from testing_gold_arch_evolution.conftest import record_result
from testing_gold_arch_evolution.retrieval import answer_question, embed_question, top_k_gold_evolution
...
def _matches_entity_and_operation(rows, expected: dict) -> bool:
    ...
    from retrieval. agents.schemas.ContractAction's own docstring already
...
async def _check_current_gold_state() -> list[str]:
    ...
    """
    async with async_session_factory() as session:
        current_rows = await gold_service.current_gold_state(session, entity_type="component")
